# --- Build the ffmpeg with CUDA support from source. We need ffmpeg on the system that works with the GPU. Only having the python package is not enough. ---
...
RUN apt-get update && \
	apt-get install -y libglib2.0-0 libsm6 libxext6 libc6 libnuma1 libmagic1 libxrender1 && \ 
